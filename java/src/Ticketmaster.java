...
public class Ticketmaster{
	...
	public static void ListMovieAndShowInfoAtCinemaInDateRange(Ticketmaster esql){//13
		...
		try{
			String query = "SELECT m.title, m.duration, s.sdate, s.sttime FROM Movies m, Shows s, Plays p, Cinemas c, Theaters t WHERE m.title = '" + movie + "' AND c.cname = '" + cinema + "' AND c.cid = t.tid AND t.tid = p.tid AND p.sid = s.sid AND s.mvid = m.mvid AND s.sdate >= '" + date1 + "' AND s.sdate <= '" + date2 + "';";
			esql.executeQueryAndPrintResult(query);
		}catch(Exception e){
			System.out.println(e);
		}
	...
	public static void ListBookingInfoForUser(Ticketmaster esql){//14
		...
		try{
			String query = "SELECT m.title, s.sdate, s.sttime, t.tname,  cs.sno FROM Movies m, Shows s, Bookings b, Users u, ShowSeats ss, CinemaSeats cs, Theaters t, Plays p WHERE b.email = '" + input + "' AND b.bid = ss.bid AND ss.sid = s.sid AND s.mvid = m.mvid AND p.sid = ss.sid AND p.tid = t.tid AND t.tid = cs.tid;"; //AND s.sid = ss.sid
			esql.executeQueryAndPrintResult(query);
		}catch(Exception e){
			System.out.println(e);
		}
