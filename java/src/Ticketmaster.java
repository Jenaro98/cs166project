...
public class Ticketmaster{
	...
	public static void ListBookingInfoForUser(Ticketmaster esql){//14
		...
		try{
			String query = "SELECT m.title, s.sdate, s.sttime, t.tname,  cs.sno FROM Plays p, Movies m, Shows s, Bookings b, Users u, Theaters t, Cinemas c, CinemaSeats cs WHERE u.email = '" + input + "' AND b.sid = s.sid AND s.mvid = m.mvid AND s.sid = p.sid AND p.tid = t.tid AND t.tid = cs.tid ;";
			esql.executeQueryAndPrintResult(query);
		}catch(Exception e){
			System.out.println(e);
		}
